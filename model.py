...
model = CLoGNet(
    in_channels=3,
    num_classes=2,
    widths=[32, 48, 80],  # match training
    blocks=[1, 1, 1],     # match training
    pool_size=5            # match training
).to(device)

# Load checkpoint
checkpoint = torch.load("clognet_weights.pth", map_location=device)

# Your checkpoint might contain a dict with multiple keys (like optimizer_state)
# If you saved the model like: torch.save({'model_state': model.state_dict(), ...})
# then load specifically:
